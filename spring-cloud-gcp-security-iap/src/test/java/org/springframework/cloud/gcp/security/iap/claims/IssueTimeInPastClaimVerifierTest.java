...
public class IssueTimeInPastClaimVerifierTest {
	...
	IssueTimeInPastClaimVerifier verifier = new IssueTimeInPastClaimVerifier();

	public IssueTimeInPastClaimVerifierTest() {
		this.verifier.setClock(this.clock);
	}

	@Test
	public void issueTimeInPastSucceeds() {
		Date pastDateTime = Date.from(this.fakeDateTime.minusMinutes(1).toInstant(ZoneOffset.UTC));
		assertTrue(this.verifier.verify(makeClaimsSet(pastDateTime)));
	}

	@Test
	public void issueTimeRightNowFails() {
	...
	@Test
	public void issueTimeInFutureFails() {
		Date futureDateTime = Date.from(this.fakeDateTime.plusMinutes(1).toInstant(ZoneOffset.UTC));
		assertFalse(this.verifier.verify(makeClaimsSet(futureDateTime)));
	}

	private JWTClaimsSet makeClaimsSet(Date date) {
		return new JWTClaimsSet.Builder().issueTime(date).build();
	}
}
