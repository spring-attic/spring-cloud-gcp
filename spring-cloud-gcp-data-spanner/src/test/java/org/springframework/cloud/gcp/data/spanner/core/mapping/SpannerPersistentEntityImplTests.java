...
public class SpannerPersistentEntityImplTests {
	...
	public void testDuplicateEmbeddedColumnName() {
		this.thrown.expect(SpannerDataException.class);
		this.thrown.expectMessage("Two properties resolve to the same column name: "
	...
	public void testEmbeddedParentKeys() {
		...
		assertThat(key).isEqualTo(Key.newBuilder().append("1").append("2").append("3")
				.append("4").appendObject(null).build());
	}
	...
	public void testEmbeddedCollection() {
		...
		this.thrown
				.expectMessage("org.springframework.cloud.gcp.data.spanner.core.mapping."
	...
	public void doWithChildrenCollectionsTest() {
		PropertyHandler<SpannerPersistentProperty> mockHandler = mock(
				PropertyHandler.class);
		SpannerPersistentEntity spannerPersistentEntity = this.spannerMappingContext
				.getPersistentEntity(ParentInRelationship.class);
		doAnswer((invocation) -> {
			String colName = ((SpannerPersistentProperty) invocation.getArgument(0))
					.getName();
			assertThat(colName.equals("childrenA") || colName.equals("childrenB"))
					.isTrue();
			return null;
	...
	private static class ParentEmbedded {
		...
		// This property will be stored as a STRING column even though Long is a natively
		// supported type.
