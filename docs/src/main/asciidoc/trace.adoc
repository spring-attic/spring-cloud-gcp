...
| `spring.cloud.gcp.trace.messageTimeout` | Timeout in seconds before pending spans will be sent in batches to GCP Stackdriver Trace. Added for forward compatibility. | No | `spring.zipkin.messageTimeout`
|===
