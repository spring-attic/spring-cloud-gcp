...
- Spring Cloud GCP Trace always uses 128-bit Trace IDs. This is required by Stackdriver Trace.
- Spring Cloud GCP provides a `HttpTracing` configuration that behaves more like Spring Cloud Sleuth 1.x.
  Effectively, it's the same behavior as always setting `spring.sleuth.http.legacy=true`.
