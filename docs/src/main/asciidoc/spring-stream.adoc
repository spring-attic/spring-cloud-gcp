...
WARNING: When using the `spring-cloud-gcp-pubsub-stream-binder` dependency, configuring `spring.cloud.function.definition` property to indicate function bean is *required*, even though Spring Cloud Stream has ability to autodiscover functional beans.
