...
When return type is Slice or Pageable, the cursor that points to position after the result is preserved. In order to leverage it to make queries more efficient, use Pageable produced by `slice.getPageable().next()` (same for Page).

NOTE: `Page` requires total count of entities produced by the query, therefore the first query will have to retrieve all the records just to count them. Instead, we recommend using `Slice` return type, because it does not require a count query.
