...
They enable null checks during runtime, as follows:
 ...
 . `@NonNull`: Used on a return value that must not be null (the repository class has to be annotated with `@javax.annotation.Nonnull`).

 . `@Nullable`: Used on a return value that can be `null` (useful if you enable `@NonNullApi`, but still want some methods renurn values or parameters to be null).

NOTE: `@NonNull` and `@Nullable` can be used on parameters as well.
...
Once the checking is enabled, repository query method invocations get validated at runtime for nullability constraints.
If a query execution result is `null` but it is declared as non-nullable, an exception is thrown.
In case you want to allow some methods to return `null`, you can use `@Nullable` on individual methods.
