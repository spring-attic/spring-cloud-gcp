...
It enables dynamic query generation based on user-provided object. See https://docs.spring.io/spring-data/jpa/docs/current/reference/html/#query-by-example[Spring Data Documentation] for details.
...
----
`null` fields are not used in the filter by default. If you want to include them, you would use following code:
[source, java]
----
userRepository.findAll(
