...
When reading and querying entities using `DatastoreRepository` or `DatastoreTemplate` with a superclass as the type parameter, you can receive instances of subclasses if you annotate the superclass and its subclasses with `DiscriminationField` and `DiscriminationValue`:

[source, java]
----
@Entity(name = "pets")
@DiscriminationField(field = "pet_type")
@DiscriminationValue("pet")
abstract class Pet {
	@Id
	Long id;
...
Instances of all 4 types are stored in the `pets` Kind.
...
Entity rows in Cloud Datastore store their respective types' `DiscriminationValue` in a field specified by the root superclass's `DiscriminationField` (`pet_type` in this case).
Reads and queries using a given type parameter will match each entity with its specific type.
For example, reading a `List<Pet>` will produce a list containing instances of all 4 types.
...
annotated with `DiscriminationField`.
