...
It uses a user-provided object as an example and generates query based on it and allows users to choose which fields should be excluded from filter. See https://docs.spring.io/spring-data/jpa/docs/current/reference/html/#query-by-example[Spring Data Documeentation] for details.

NOTE: Currently, only equality queries are supported (no ignore-case matching, regexp matching, etc.).
NullHandler.INCLUDE is not supported.
Per-field matchers are not supported.
Embedded entities matching is not supported.

For example, this call will match all fields, except `id` and `firstName` (assuming there are only `id`, `firstName` and `lastName`, it will find all users with the last name equal to "Smith"):
[source, java]
----
userRepository.findAll(
    Example.of(
        new User(null, null, "Smith"),
        ExampleMatcher.matching().withIgnorePaths("id", "firstName"))))
----


This one will match all fields from object (basically validating that such object has been saved to Cloud Datastore):
[source, java]
----
userRepository.findAll(Example.of(new User(1, "John", "Smith")));
----
