...
To pull multiple messages on each request, use the `setMaxFetchSize()` method to set the max number of messages to pull.
...
Messages can only be retrieved one at a time from a `PubSubMessageSource`. To retrieve a batch of messages, use `PubSubBatchMessageSource`.
...
Payload type is not configurable for `PubSubBatchMessageSource`, since batched messages are delivered as a list of `AcknowledgeablePubsubMessage` objects.
