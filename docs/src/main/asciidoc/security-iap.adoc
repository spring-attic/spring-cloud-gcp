...
The audience (`"aud"`) validation is automatically configured when the application is running on AppEngine.
...
To use the IAP starter on GCE/GKE, find the Audience string per instructions the https://cloud.google.com/iap/docs/signed-headers-howto#verify_the_jwt_payload[Verify the JWT payload] guide, and specify it in `spring.cloud.gcp.security.iap.audience` property.
...
|false on AppEngine; true on GCE/GKE
|
