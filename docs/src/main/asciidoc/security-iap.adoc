...
IMPORTANT: The audience check is *not* enabled by default on Compute Engine or Kubernetes Engine. To fully secure an application running on Compute Engine or Kubernetes Engine, find the Audience string per instructions the https://cloud.google.com/iap/docs/signed-headers-howto#verify_the_jwt_payload[Verify the JWT payload] guide, and specify it in `spring.cloud.gcp.security.iap.audience` property.
