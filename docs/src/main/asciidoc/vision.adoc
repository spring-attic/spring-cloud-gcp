...
The storage starter is necessary because the Cloud Vision API runs OCR on documents stored in Google Cloud Storage buckets.
...
It is recommended to register callbacks to the returned ListenableFuture or ignore it and process the JSON output files at a later point in time.
