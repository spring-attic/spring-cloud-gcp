...
The Spring Cloud GCP Config support is provided via the Spring Cloud GCP Autoconfigure module.
It enables the use of the `@ConfigurationProperties` annotation to map a class's fields to settings
stored by the Google Runtime Configuration API.
