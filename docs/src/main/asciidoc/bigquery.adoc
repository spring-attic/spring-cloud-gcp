...
This offers a convenient way to write pipelines where a goal is to load data into BigQuery.
...
On the provided output channel, the handler will return a https://googleapis.dev/java/google-cloud-clients/latest/com/google/cloud/bigquery/Job.html[Job] object or a `ListenableFuture` of the job as a reply for each file that is loaded.
...
This default can be modified by changing the dataset, table name, and format option expressions in `BigQueryFileMessageHandler`.
