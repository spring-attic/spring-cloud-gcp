...
Below is an example configurating a `ServiceActivator` bean using the `BigQueryFileMessageHandler`.
...
Alternatively, you may omit these headers and explicitly set the table name or format options explicitly by calling `setTableName(...)` and `setFormatOptions(...)`.
...
NOTE: If you decide to use Spring Integration Gateways and you wish to receive `ListenableFuture<Job>` as a reply object in the Gateway, you will have to call `.setAsyncExecutor(null)` on your `GatewayProxyFactoryBean` for the gateway.
