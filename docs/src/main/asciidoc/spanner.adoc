...
The `Trade` type parameter to `CrudRepository` refers to the underlying domain type.
...
`curl \http://<server>:<port>/trades/<sequence>,<trader_id>`.

The separator between your primary key components, `sequence` and `trader_id` in this case, is a comma
