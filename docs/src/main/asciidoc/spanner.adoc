...
Using a https://cloud.google.com/spanner/docs/secondary-indexes[secondary index] is only available for Reads.
...
//...
  ...
  SpannerReadOptions spannerReadOptions = newn SpannerReadOptions().setIndex("TradesByTrader");
  List<Trade> trades = this.spannerTemplate.readAll(Trade.class, spannerReadOptions);
...
Partial read is only possible when using Queries. In case the rows returned by query have less columns than the entity that it will be mapped to, Spring Data will map the returned columns and leave the rest as they of the columns are.
