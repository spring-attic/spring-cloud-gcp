...
Operations expecting List arguments:

* `contains` (allows for combining up to 10 array-contains clauses on the same field with a logical OR)
* `in` (allows for combining up to 10 equality clauses on the same field with a logical OR)

NOTE: Only one of `contains` and `in` operations are allowed in a query.
