...
NOTE: If the property annotated with `@DocumentId` is `null` and the property type is `String`, the document id is generated automatically when the entity is saved.
...
public class FirestoreTemplateExample {
	...
	public Flux<User> findUsers() {
		return this.firestoreTemplate.findAll(User.class);
	}

	public Mono<Long> removeAllUsers() {
		return this.firestoreTemplate.deleteAll(User.class);
	}
}
----
