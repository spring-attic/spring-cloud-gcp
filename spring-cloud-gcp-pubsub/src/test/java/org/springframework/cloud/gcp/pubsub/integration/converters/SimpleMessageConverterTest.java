...
public class SimpleMessageConverterTest {
  ...
  public void testFromMessage() {
    ...
    PubsubMessage pubsubMessage =
        (PubsubMessage) converter.fromMessage(message, PubsubMessage.class);
    assertEquals("test payload", pubsubMessage.getData().toStringUtf8());
  }

  @Test(expected = MessageConversionException.class)
  public void testFromMessage_notPubsubMessage() {
    new SimpleMessageConverter().fromMessage(
        new GenericMessage<Object>("payload"), String.class);
  }
  ...
  public void testToMessage_noHeaders() {
    SimpleMessageConverter converter = new SimpleMessageConverter();
    String payload = "test payload";
    Message<?> message = converter.toMessage(payload, new MessageHeaders(new HashMap<>()));
    assertEquals("test payload", message.getPayload());
  }

  @Test(expected = MessageConversionException.class)
  public void testToMessage_notString() {
    new SimpleMessageConverter().toMessage(new HashMap<>(), new MessageHeaders(new HashMap<>()));
  }
}
