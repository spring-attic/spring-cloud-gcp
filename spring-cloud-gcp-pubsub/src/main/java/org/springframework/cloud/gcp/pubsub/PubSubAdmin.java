...
public class PubSubAdmin implements AutoCloseable {
	...
	/**
	 * This constructor instantiates TopicAdminClient and SubscriptionAdminClient with all
	 * their defaults and the provided credentials provider.
	...
	public Topic createTopic(String topicName) {
		...
		return this.topicAdminClient
				.createTopic(ProjectTopicName.of(this.projectId, topicName));
	}
	...
	public Topic getTopic(String topicName) {
		...
		try {
			return this.topicAdminClient
					.getTopic(ProjectTopicName.of(this.projectId, topicName));
		}
	...
	public List<Topic> listTopics() {
		TopicAdminClient.ListTopicsPagedResponse topicListPage = this.topicAdminClient
				.listTopics(ProjectName.of(this.projectId));
	 ...
	 * @param subscriptionName the name of the new subscription
	 * @param topicName the name of the topic being subscribed to
	 * @param ackDeadline deadline in seconds before a message is resent, must be between
	 * 10 and 600 seconds. If not provided, set to default of 10 seconds
	 * @return the created subscription
	 */
	public Subscription createSubscription(String subscriptionName, String topicName,
			Integer ackDeadline) {
	...
	public Subscription createSubscription(String subscriptionName, String topicName,
		...
		return this.subscriptionAdminClient.createSubscription(
				...
				ProjectTopicName.of(this.projectId, topicName), pushConfigBuilder.build(),
				finalAckDeadline);
	}
	...
	public List<Subscription> listSubscriptions() {
		SubscriptionAdminClient.ListSubscriptionsPagedResponse subscriptionsPage = this.subscriptionAdminClient
				.listSubscriptions(ProjectName.of(this.projectId));
	 ...
	 * @param defaultAckDeadline default acknowledgement deadline value in seconds, must
	 * be between 10 and 600 seconds.
	 */
	...
	private void validateAckDeadline(int ackDeadline) {
		Assert.isTrue(
				ackDeadline >= MIN_ACK_DEADLINE_SECONDS
						&& ackDeadline <= MAX_ACK_DEADLINE_SECONDS,
				"The acknowledgement deadline must be between " + MIN_ACK_DEADLINE_SECONDS
						+ " and " + MAX_ACK_DEADLINE_SECONDS + " seconds.");
	}
