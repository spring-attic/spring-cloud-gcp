...
public class DefaultDatastoreEntityConverterTests {
	...
	private static final DatastoreEntityConverter ENTITY_CONVERTER = new DefaultDatastoreEntityConverter(
	...
	public void readTest() {
		...
		Entity entity = getEntityBuilder().set("durationField", "PT24H")
				...
				.set("blobField", Blob.copyFrom(bytes)).set("intField", 99)
				.set("enumField", "WHITE").set("keyField", otherKey).build();
		TestDatastoreItem item = ENTITY_CONVERTER.read(TestDatastoreItem.class, entity);

		assertThat(item.getDurationField()).as("validate duration field")
				.isEqualTo(Duration.ofDays(1));
		assertThat(item.getStringField()).as("validate string field")
		...
		assertThat(item.getBlobField()).as("validate blob field")
				.isEqualTo(Blob.copyFrom(bytes));
		assertThat(item.getIntField()).as("validate int field").isEqualTo(99);
		assertThat(item.getEnumField()).as("validate enum field")
	...
	public void discriminatingReadTest() {
		Entity entityA = getEntityBuilder()
				...
				.set("boolField", true).set("intField", 99).set("enumField", "WHITE")
				.build();
		...
		// All the reads use the superclass type but verify to be instances of the
		// subclasses.
		...
		// Because A is NOT a superclass of X, we cannot read entityX as type X. It falls
		// back to
		// A.
	...
	public void readNullTest() {
		byte[] bytes = { 1, 2, 3 };
		Entity entity = getEntityBuilder().set("durationField", "PT24H")
				.set("stringField", new NullValue()).set("boolField", true)
				.set("doubleField", 3.1415D).set("longField", 123L)
				.set("latLngField", LatLng.of(10, 20))
				.set("timestampField", Timestamp.ofTimeSecondsAndNanos(30, 40))
				.set("blobField", Blob.copyFrom(bytes)).set("intField", 99)
				.set("enumField", "BLACK").build();
		TestDatastoreItem item = ENTITY_CONVERTER.read(TestDatastoreItem.class, entity);
	...
	public void testWrongTypeReadException() {
		...
		Entity entity = getEntityBuilder().set("stringField", "string value")
	...
	public void testObjectEntityException() {
		this.thrown.expect(DatastoreDataException.class);
		this.thrown.expectMessage(
				"Unable to convert Datastore Entity to class java.lang.Object");
	...
	public void testWrongTypeReadExceptionList() {
		...
		Entity entity = getEntityBuilder().set("stringField", "string value")
				.set("boolField", ListValue.of(true)).build();
	...
	public void writeTest() {
		...
		assertThat(entity.getDouble("doubleField")).as("validate double field")
				.isEqualTo(3.1415D);
		...
		assertThat(entity.getString("enumField")).as("validate enum field")
				.isEqualTo("BLACK");
	...
	public void writeTestSubtypes() {
		...
		assertThat(entity.getList("discrimination_column"))
				.as("validate discrimination field").containsExactly(StringValue.of("D"),
						StringValue.of("B"), StringValue.of("X"));
	}
	...
	public void testUnsupportedTypeWriteException() {
		this.thrown.expect(DatastoreDataException.class);
		this.thrown.expectMessage(
				"Unable to write testItemUnsupportedFields.unsupportedField");
		this.thrown.expectMessage("Unable to convert class "
	...
	public void testUnsupportedTypeWrite() {
		...
		DatastoreEntityConverter entityConverter = new DefaultDatastoreEntityConverter(
				...
				new TwoStepsConversions(new DatastoreCustomConversions(Arrays.asList(
						getIntegerToNewTypeConverter(), getNewTypeToIntegerConverter())),
						null));
		Entity.Builder builder = getEntityBuilder();
		...
		TestItemUnsupportedFields readItem = entityConverter
				.read(TestItemUnsupportedFields.class, entity);

		assertThat(item.equals(readItem)).as("read object should be equal to original")
				.isTrue();
	}
	...
	public void testCollectionFieldsUnsupportedCollection() {
		...
		this.thrown.expectMessage("Failed to convert from type [java.util.ArrayList<?>] "
		...
		TestDatastoreItemCollections item = new TestDatastoreItemCollections(
				Arrays.asList(1, 2), comparableBeanContextSupport,
		...
		TestDatastoreItemCollections result = ENTITY_CONVERTER
				.read(TestDatastoreItemCollections.class, entity);
	}
	...
	public void testCollectionFields() {
		...
		TestDatastoreItemCollections item = new TestDatastoreItemCollections(
				Arrays.asList(1, 2), ComparableBeanContextSupport,
				new String[] { "abc", "def" }, new boolean[] { true, false }, bytes,
				listByteArray);
		...
		DatastoreEntityConverter entityConverter = new DefaultDatastoreEntityConverter(
				...
				new TwoStepsConversions(new DatastoreCustomConversions(Arrays.asList(
						new Converter<List<String>, ComparableBeanContextSupport>() {
							@Override
							public ComparableBeanContextSupport convert(
									List<String> source) {
							...
							public List<String> convert(
									ComparableBeanContextSupport bcs) {
								List<String> list = new ArrayList<>();
								bcs.iterator()
										.forEachRemaining((s) -> list.add((String) s));
								return list;
							}
						})), null));
		...
		assertThat(beanContext.stream().map(Value::get).collect(Collectors.toSet()))
				.as("validate bean context values").isEqualTo(
						new HashSet<>(Arrays.asList("this implementation of Collection",
		...
		assertThat(bytesVals.stream().map(Value::get).collect(Collectors.toList()))
				...
				.isEqualTo(Arrays.asList(Blob.copyFrom(new byte[] { 1, 2 }),
						Blob.copyFrom(new byte[] { 3, 4 })));

		List<Value<?>> listByteArrayVals = entity.getList("listByteArray");
		assertThat(
				listByteArrayVals.stream().map(Value::get).collect(Collectors.toList()))
						.as("validate list of byte[]")
						.isEqualTo(Arrays.asList(Blob.copyFrom(new byte[] { 1, 2 }),
								Blob.copyFrom(new byte[] { 3, 4 })));

		TestDatastoreItemCollections readItem = entityConverter
				.read(TestDatastoreItemCollections.class, entity);
	...
	public void testCollectionFieldsNulls() {
		TestDatastoreItemCollections item = new TestDatastoreItemCollections(
				Arrays.asList(1, 2), null, null, new boolean[] { true, false }, null,
				null);
		...
		TestDatastoreItemCollections readItem = ENTITY_CONVERTER
				.read(TestDatastoreItemCollections.class, entity);
		assertThat(item.equals(readItem)).as("read object should be equal to original")
				.isTrue();
	...
	public void testCollectionFieldsUnsupported() {
		this.thrown.expect(DatastoreDataException.class);
		this.thrown.expectMessage(
				"Unable to write collectionOfUnsupportedTypes.unsupportedElts");
	...
	public void testCollectionFieldsUnsupportedWriteReadException() {
		...
		entityConverter.read(TestItemUnsupportedFields.CollectionOfUnsupportedTypes.class,
				entity);
	}
	...
	public void testCollectionFieldsUnsupportedWriteRead() {
		...
		TestItemUnsupportedFields.CollectionOfUnsupportedTypes read = entityConverter
				.read(TestItemUnsupportedFields.CollectionOfUnsupportedTypes.class,
						entity);

		assertThat(read.equals(item)).as("read object should be equal to original")
				.isTrue();

	}

	private TestItemUnsupportedFields.CollectionOfUnsupportedTypes getCollectionOfUnsupportedTypesItem() {
		TestItemUnsupportedFields.CollectionOfUnsupportedTypes item = new TestItemUnsupportedFields.CollectionOfUnsupportedTypes();

		item.getUnsupportedElts()
				.addAll(Arrays.asList(new TestItemUnsupportedFields.NewType(true),
	...
	public void testEmbeddedEntity() {
		...
		assertThat(entity.getList("listOfEmbeddedEntities").stream()
				...
				.collect(Collectors.toList())).as("validate embedded entity")
		...
		assertThat(read.getNestedEmbeddedMaps().get("outer1").get(1L).get("a"))
				.isEqualTo("valueA");
		assertThat(read.getNestedEmbeddedMaps().get("outer1").get(1L).get("b"))
				.isEqualTo("valueB");
