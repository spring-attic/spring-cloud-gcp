...
you will need to specify the trace ID in the `x-cloud-trace-context` or `X-B3-TraceId` headers manually to see a trace ID
...
in the `x-cloud-trace-context` header. See the link:../../spring-cloud-gcp-starters/spring-cloud-gcp-starter-logging[Spring Cloud GCP Logging Starter]
