...
Technically, every configuration server happens to also be a configuration client, but by configuration propagation is suppressed on the server by default.
