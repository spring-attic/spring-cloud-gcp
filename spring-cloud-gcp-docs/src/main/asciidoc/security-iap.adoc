...
Additionally, Audience (`"aud"`) validation is enabled when the application is running on AppEngine/ComputeEngine/Kubernetes, or when a custom audience is provided through `spring.cloud.gcp.security.iap.audience` property. The custom property overrides automatic environment-based checks.
...
|Custom JWK audience to verify. Note that without this option, the audience is automatically determined for applications deployed to App Engine/Compute Engine/Kubernetes Engine.
|false
|
