...
This module provides support for associating a web request's trace ID with the corresponding log entries.
...
This appender builds a Stackdriver Logging log entry from a JUL or Logback log entry, adds a trace ID to it from the MDC and sends it to Stackdriver Logging.
The trace ID is extracted from the MDC using the `X-B3-TraceId` key.
...
Therefore, you can just include `org/springframework/cloud/gcp/autoconfigure/logging/logback-json-appender.xml` in your logging configuration.
