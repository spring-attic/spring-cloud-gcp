...
public class Item {
  ...
  Timestamp originalAcquisitionTime;
}
----
...
Properties holding child entities must be collection-like, but they can be any of the supported types inter-convertible collection-like types that are supported for regular properties such as `List`, arrays, `Set`, etc...
...
Children entities cannot be moved from the property of one parent to that of another unless the child's key property is set to null or a value that contains the new parent as an ancestor.
