...
`DatastoreRepository` and custom defined entitiy repositories implement Spring Data's `PagingAndSortingRepository`, which supports offsets and limits using page numbers and page sizes.
Paging and sorting options are also supported in `DatastoreTemplate` by supplying a `DatastoreQueryOptions`.
...
If using custom GQL queries, you can further restrict the fields retrieved from Datastore to just those required by the projection.
