...
If your domain type is only a subset of columns of the corresponding Cloud Datastore Kind, you can annotate the query method `@Query(runAsProjection = true)` to run the query as a projection query.
Note that there are https://cloud.google.com/datastore/docs/concepts/queries#limitations_on_projections[limitations] to this type of query, one consequence of which is that the properties used to filter in the query method will all be null in the result.
...
If you want to specify the selected columns instead of using `SELECT *`, you must set `runAsProjection = true` in the `@Query` annotation.
This causes the query to be run as a Cloud Datastore Projection query and has https://cloud.google.com/datastore/docs/concepts/queries#limitations_on_projections[limitations].
