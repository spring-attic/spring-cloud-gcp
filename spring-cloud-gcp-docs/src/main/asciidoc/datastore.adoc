...
Fields marked as `@Embedded` are converted to Datastore entities and stored inside the parent entity.
...
(each node can have arbitrary number of children) in Cloud Datastore.
