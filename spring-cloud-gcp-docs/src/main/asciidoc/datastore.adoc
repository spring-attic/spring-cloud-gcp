...
See https://docs.spring.io/spring-data/data-commons/docs/current/reference/html/#repositories.query-methods[according Spring Data documentation] for details.

If a method's return type is `org.springframework.data.domain.Slice`, the return object besides results will have additional information (e.g. if there the next slice exists).
...
This additional information is only reliable if used within a transaction.
