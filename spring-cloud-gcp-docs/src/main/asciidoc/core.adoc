...
You can also use `STARTER_SCOPES` placeholder as a scope to represent the starters default scopes,
and append the additional scopes you need to add.

[source,yaml]
----
spring.cloud.gcp.credentials.scopes=STARTER_SCOPES,https://www.googleapis.com/auth/cloud-vision
----
