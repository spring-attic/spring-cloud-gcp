...
Spring Data Cloud Spanner supports one-to-many relationships using the Cloud Spanner https://cloud.google.com/spanner/docs/schema-and-data-model#creating-interleaved-tables[parent-child interleaved
...
interleaved parent-child tables, only the one-to-many relationship is natively supported.
...
These classes can correspond to an existing pair of interleaved tables. The `@OneToMany` annotation may be applied to
...
List<String> createStrings = this.schemaUtils.getCreateTableDdlStringsForHierarchy(Singer.class);
...
The `ON DELETE CASCADE` phrase indicates that Cloud Spanner will delete all Albums of a singer if the Singer is deleted.
