...
| `spring.cloud.gcp.spanner.numRpcChannels` | Number of gRPC channels used to connect to Spanner | No | 4 - Determined by Spanner client library
| `spring.cloud.gcp.spanner.prefetchChunks` | Number of chunks prefetched by Spanner for read and query | No | 4 - Determined by Spanner client library
| `spring.cloud.gcp.spanner.minSessions` | Minimum number of sessions maintained in the session pool | No | 0 - Determined by Spanner client library
| `spring.cloud.gcp.spanner.maxSessions` | Maximum number of sessions session pool can have | No | 400 - Determined by Spanner client library
| `spring.cloud.gcp.spanner.maxIdleSessions` | Maximum number of idle sessions session pool will maintain | No | 0 - Determined by Spanner client library
| `spring.cloud.gcp.spanner.writeSessionsFraction` | Fraction of sessions to be kept prepared for write transactions | No | 0.2 - Determined by Spanner client library
| `spring.cloud.gcp.spanner.keepAliveIntervalMinutes` | How long to keep idle sessions alive | No | 30 - Determined by Spanner client library
|===
...
If no read `Converter` with a target type of `Struct` is provided, Spring Data Cloud Spanner
will attempt to read the column STRUCT values into a property that is an `Iterable`
of an entity type compatible with the schema of the column STRUCT value. For example: `List<TwoInts> pair;`
...
If a `Converter<Struct, A>` is provided, then properties of type `List<A>` can be used.
