 ...
 *
 * Tests can access the emulator's host/port combination by calling
 * {@link #getEmulatorHostPort()} method.
 *
 * @author Elena Felder
...
public class PubSubEmulator extends ExternalResource {
	...
	private static final Path EMULATOR_CONFIG_DIR = Paths
			.get(System.getProperty("user.home"))
			.resolve(Paths.get(".config", "gcloud", "emulators", "pubsub"));

	private static final String ENV_FILE_NAME = "env.yaml";

	private static final Path EMULATOR_CONFIG_PATH = EMULATOR_CONFIG_DIR
			.resolve(ENV_FILE_NAME);
	...
	public PubSubEmulator() {
		if ("true".equals(System.getProperty("it.pubsub-emulator"))) {
			...
			LOGGER.warn(
					"PubSubEmulator rule disabled. Please enable with -Dit.pubsub-emulator.");
		}
	}

	/**
	 * Launch an instance of pubsub emulator or skip all tests. If it.pubsub-emulator
	 * environmental property is off, all tests will be skipped through the failed
	 * assumption. If the property is on, any setup failure will trigger test failure.
	 * Failures during teardown are merely logged.
	...
	/**
	 * Shut down the two emulator processes. gcloud command is shut down through the
	 * direct process handle. java process is identified and shut down through shell
	 * commands. There should normally be only one process with that host/port
	 * combination, but if there are more, they will be cleaned up as well. Any failure is
	 * logged and ignored since it's not critical to the tests' operation.
	 */
	@Override
	...
	private void findAndDestroyEmulator() {
		...
		if (this.emulatorHostPort == null) {
			...
			String hostPortParams = String.format("--host=%s --port=%s", emulatorHost,
					emulatorPort);
			try {
				...
				try (BufferedReader br = new BufferedReader(
					...
					br.lines().filter((psLine) -> psLine.contains(hostPortParams))
				...
				if (!foundEmulatorProcess.get()) {
					LOGGER.warn("Did not find the emualtor process to kill based on: "
							+ hostPortParams);
				}
			}
	...
	/**
	 * Return the already-started emulator's host/port combination when called from within
	 * a JUnit method.
	...
	private void startEmulator() throws IOException, InterruptedException {
		...
		if (configPresent) {
			...
			EMULATOR_CONFIG_DIR.register(watchService,
					StandardWatchEventKinds.ENTRY_MODIFY);
		}

		try {
			this.emulatorProcess = new ProcessBuilder("gcloud", "beta", "emulators",
					"pubsub", "start").start();
		}
		catch (IOException ex) {
	...
	private void determineHostPort() throws IOException, InterruptedException {
		Process envInitProcess = new ProcessBuilder("gcloud", "beta", "emulators",
				"pubsub", "env-init").start();

		try (BufferedReader br = new BufferedReader(
				new InputStreamReader(envInitProcess.getInputStream()))) {
			String emulatorInitString = br.readLine();
			envInitProcess.waitFor();
			this.emulatorHostPort = emulatorInitString
					.substring(emulatorInitString.indexOf('=') + 1);
		}
	}

	/**
	 * Wait until a PubSub emulator configuration file is present. Fail if the file does
	 * not appear after 10 seconds.
	 * @throws InterruptedException which should interrupt the peaceful slumber and bubble
	 * up to fail the test.
	 */
	private void createConfig() throws InterruptedException {
		...
		if (attempts < 0) {
			fail("Emulator could not be configured due to missing env.yaml. Are PubSub and beta tools installed?");
		}
	}

	/**
	 * Wait until a PubSub emulator configuration file is updated. Fail if the file does
	 * not update after 1 second.
	...
	/**
	 * Attempt to kill a process on best effort basis. Failure is logged and ignored, as
	 * it is not critical to the tests' functionality.
