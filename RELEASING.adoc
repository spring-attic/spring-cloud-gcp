...
. Increment the project base _patch_ version. For example, from `2.0.3-SNAPSHOT` to `2.0.4-SNAPSHOT`, do:
```
$ ./mvnw versions:set -DprocessAllModules -DnewVersion=2.0.4-SNAPSHOT
```
...
2. Add a new Github SCM and fill out the following parameters:
    ...
    - Committish: Set this to the name of the new release branch or tag that you want to publish javadocs for (like `1.2.x` or `v2.0.3`).
...
----
+
Example: If you published the javadocs for version `2.0.3-SNAPSHOT`, then the URL would be: https://googleapis.dev/java/spring-cloud-gcp/2.0.3-SNAPSHOT/index.html
